...
{
    public static ProcessDef ProcessFromSmelterRecipe(string prefix, int tickMultiplier, ProcessorTemplateDef tp,
        ...
        if (def.stuffCategories?.Contains(StuffCategoryDefOf.Metallic) == true || def.stuffCategories?.Contains(MPEDefOf.Steeled) == true) {
        ...
        if (def.recipeMaker?.researchPrerequisites != null) {
            ...
            processDef.researchPrerequisites = [def.recipeMaker.researchPrerequisite];
        }
